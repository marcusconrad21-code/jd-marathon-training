...
import { buildPlan, fmtD, PHASE_META, WIGGLE, YOUR_PACES, DAY_NAMES, addDays } from './plan.js'
    ...
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.52)',display:'flex',alignItems:'flex-end',justifyContent:'center',zIndex:100}} onClick={onClose}>
      <div style={{background:'#fff',width:'100%',maxWidth:440,borderRadius:'20px 20px 0 0',padding:'22px',maxHeight:'90vh',overflowY:'auto'}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{background:'#F4F1EB',borderRadius:12,padding:'12px 14px',marginBottom:16}}>
          <div style={{fontWeight:700,fontSize:13,marginBottom:8}}>Your Target Paces</div>
          {[{label:'Easy',val:`${YOUR_PACES.E.low}–${YOUR_PACES.E.high}/mi`},{label:'Marathon',val:`${YOUR_PACES.M.low}–${YOUR_PACES.M.high}/mi`},{label:'Threshold',val:`${YOUR_PACES.T.low}–${YOUR_PACES.T.high}/mi`},{label:'Interval',val:`${YOUR_PACES.I.per1k}/km`},{label:'Reps',val:`200: ${YOUR_PACES.R.per200} / 400: ${YOUR_PACES.R.per400}`}].map(p=>(
            <div key={p.label} style={{display:'flex',justifyContent:'space-between',fontSize:12,marginBottom:4}}>
              ...
              <span style={{fontFamily:"'IBM Plex Mono',monospace",fontWeight:600}}>{p.val}</span>
            </div>
          ))}
        </div>
...
// ─── Draggable row — when dropped, BOTH workout AND date/day move ─
function DraggableRow({ index, enabled, onDrop, children }) {
  ...
  return (
    <div
      ...
      onDrop={e=>{e.preventDefault();const from=parseInt(e.dataTransfer.getData('text/plain'));if(from!==index)onDrop(from,index);setIsOver(false)}}
...
export default function App() {
  ...
  // workoutOrder[wi] = array of day objects in the user's chosen order
  // Each element is the FULL day object (workout + date + dayname) reordered together
  ...
  useEffect(()=>{
    try {
      ...
      const o=JSON.parse(localStorage.getItem('plan:orders')||'null'); if(o)setWeekOrders(o)
    } catch {}
    setLoaded(true)
  },[])
  ...
  // Apply custom ordering — when reordered, EVERYTHING moves: workout + day name + date
  // So "Thu MP run" dragged to Monday becomes "Mon MP run" on Monday's date
  const plan=useMemo(()=>({
    ...basePlan,
    weeks: basePlan.weeks.map((wk,wi)=>{
      const order=weekOrders[wi]
      if(!order||order.length!==wk.days.length) return wk
      // Rebuild days: slot i gets workout from order[i], but date/name from the SLOT position
      const newDays=order.map((srcIdx,slotIdx)=>{
        const srcDay=wk.days[srcIdx]      // workout content
        const slotDate=addDays(wk.startDate, slotIdx) // actual calendar date for this slot
        return {
          ...srcDay,                            // workout type, label, desc, miles, code
          name: DAY_NAMES[slotIdx],            // day name follows the slot (Mon=0, Tue=1…)
          date: fmtD(slotDate),               // calendar date follows the slot
  ...
  // Reorder: moves workout into a new slot; day name + date follow the slot, not the workout
  const handleReorder=useCallback((wi,fromSlot,toSlot)=>{
    if(fromSlot===toSlot) return
    const wk=basePlan.weeks[wi]
    const curOrder=weekOrders[wi]||wk.days.map((_,i)=>i) // indices of original days
    const next=[...curOrder]
    const [moved]=next.splice(fromSlot,1)
    next.splice(toSlot,0,moved)
    setWeekOrders(prev=>{const n={...prev,[wi]:next};save('plan:orders',n);return n})
  },[basePlan,weekOrders,save])
  ...
  const handleSaveSettings=useCallback(draft=>{
    ...
    setVdot(v);setPeakMi(p);setRaceDate(r);setWeekOrders({});setShowSettings(false)
    save('plan:settings',{vdot:v,peakMi:p,raceDate:r});save('plan:orders',{})
      ...
      <div style={{background:'#20242B',color:'#F4F1EB',padding:'20px 18px 18px'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
          <div>
            ...
            <div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:11,color:'#8A8F96',marginTop:5}}>days to Charlotte Marathon · {basePlan.raceDate.toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'})}</div>
          </div>
        ...
        <div style={{background:'#fff',border:'1px solid #E4DFD4',borderRadius:12,padding:'14px 14px 10px'}}>
          <div style={{height:48,display:'flex',alignItems:'flex-end',gap:2}}>
            {plan.weeks.map((wk,i)=>{
              ...
              return <div key={i} title={`Wk ${wk.weekNum}: ${wk.totalMiles}mi`} style={{flex:1,height:h,background:color,opacity:i===todayWkIdx?1:0.7,borderRadius:'3px 3px 0 0',outline:i===todayWkIdx?'2px solid #20242B':'none',outlineOffset:1,cursor:'pointer'}} onClick={()=>setExpanded(p=>({...p,[i]:true}))}/>
            })}
          </div>
            ...
            <div key={wi} style={{background:'#fff',border:`1px solid ${isNow?ph.color:'#E4DFD4'}`,borderLeft:`${isNow?4:1}px solid ${isNow?ph.color:'#E4DFD4'}`,borderRadius:14,marginBottom:9,overflow:'hidden'}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'13px 14px',cursor:'pointer'}} onClick={()=>setExpanded(p=>({...p,[wi]:!p[wi]}))}>
                <div>
                  ...
                  <div style={{fontSize:11.5,color:'#9A9489',marginTop:2}}>{doneRuns}/{totalRuns} runs logged · {wk.weekTarget} mi target</div>
                </div>
              ...
              {isOpen&&(
                <>
                  ...
                  <div style={{borderTop:'1px solid #F0EBE1',padding:'8px 14px',display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}}>
                    ...
                    <button onClick={e=>{e.stopPropagation();setReorderMode(isReorder?null:wi)}} style={{background:isReorder?'#20242B':'none',border:`1.5px solid ${isReorder?'#20242B':'#E0DAD0'}`,color:isReorder?'#fff':'#5c574d',borderRadius:8,padding:'5px 10px',fontSize:12,fontWeight:600,cursor:'pointer',display:'flex',alignItems:'center',gap:5}}><GripVertical size={12}/>{isReorder?'Done':'Reorder workouts'}</button>
                    {weekOrders[wi]&&<button onClick={()=>{setWeekOrders(prev=>{const n={...prev};delete n[wi];save('plan:orders',n);return n})}} style={{background:'none',border:'1.5px solid #E0DAD0',borderRadius:8,padding:'5px 10px',fontSize:12,cursor:'pointer',color:'#9A9489'}}>Reset order</button>}
                  </div>
                  {isReorder&&<div style={{background:'#EDF4FF',borderTop:'1px solid #C8D8F0',padding:'8px 14px',fontSize:12,color:'#4C6B8A'}}>💡 Drag <b>≡</b> handles to reschedule workouts. The day name and date move with the workout to its new position.</div>}
                    ...
                    return (
                      <DraggableRow key={`${wi}-${di}`} index={di} enabled={isReorder} onDrop={(from,to)=>handleReorder(wi,from,to)}>
                        <div style={{display:'flex',gap:10,padding:'11px 14px',borderTop:'1px solid #F0EBE1',alignItems:'flex-start',background:isDone?'#F8F6F1':'#fff'}}>
                          {isReorder&&<div style={{color:'#C8C2B6',cursor:'grab',flexShrink:0,marginTop:4,touchAction:'none'}}><GripVertical size={16}/></div>}
